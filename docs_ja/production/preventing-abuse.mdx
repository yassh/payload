---
title: Preventing Production API Abuse [翻訳済み]
label: 不正使用の防止
order: 20
...
keywords: abuse, production, config, configuration, documentation, Content Management System, cms, headless, javascript, node, react, nextjs [翻訳済み]
---
## Introduction [翻訳済み]

Payload has built-in security best practices that can be configured to your application-specific needs.

## Limit Failed Login Attempts [翻訳済み]
...
## Cross-Site Request Forgery (CSRF) [翻訳済み]

CSRF prevention will verify the authenticity of each request to your API to prevent a malicious action from another site from authorized users. See how to configure CSRF [here](/docs/authentication/cookies#csrf-attacks). [翻訳済み]

## Cross Origin Resource Sharing (CORS) [翻訳済み]
...
First, enable email [verification](/docs/authentication/email#email-verification) when users are allowed to register new accounts and add other bot prevention services. [翻訳済み]
