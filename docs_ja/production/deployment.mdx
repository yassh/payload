---
title: Production Deployment [翻訳済み]
label: デプロイメント
order: 10
...
### Double-check and thoroughly test all Access Control [翻訳済み]

Because _**you**_ are in complete control of who can do what with your data, you should double and triple-check that you
wield that power responsibly before deploying to Production. [翻訳済み]
...
fields that may be sorted in the admin UI use the [indexSortableFields](/docs/configuration/overview) option. [翻訳済み]
...
### Persistent vs Ephemeral Filesystems [翻訳済み]
...
**Popular cloud providers with ephemeral filesystems:** [翻訳済み]

- Heroku
- DigitalOcean Apps [翻訳済み]

**Popular cloud providers with persistent filesystems:** [翻訳済み]

- DigitalOcean Droplets
- Amazon EC2
- GoDaddy
- Many other more traditional web hosts [翻訳済み]
...
### Using cloud storage providers [翻訳済み]
...
Follow the docs to configure any one of these storage providers. For local development, it might be handy to simply store uploads on your own computer, and then when it comes to production, simply enable the plugin for the cloud storage vendor of your choice. [翻訳済み]
...
Here is an example of a docker-compose.yml file that can be used for development [翻訳済み]
