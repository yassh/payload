---
title: 2.0 to 3.0 Migration Guide [翻訳済み]
label: 2.0 to 3.0 Migration Guide [翻訳済み]
order: 10
desc: Upgrade guide for Payload 2.x projects migrating to 3.0.
keywords: local api, config, configuration, documentation, Content Management System, cms, headless, javascript, node, react [翻訳済み]
---
...
If you are upgrading from a _previous beta_, please see the [Upgrade From Previous Beta](#upgrade-from-previous-beta) section. [翻訳済み]

## What has changed? [翻訳済み]
...
### Table of Contents [翻訳済み]

All breaking changes are listed below. If you encounter changes that are not explicitly listed here, please consider contributing to this documentation by submitting a PR. [翻訳済み]
...
Also install the other @payloadcms packages specific to the plugins and adapters you are using. Depending on your project, these may include: [翻訳済み]
...
- @payloadcms/db-mongodb
   ...
   - @payloadcms/plugin-cloud-storage - Read [More](#@payloadcms/plugin-cloud-storage). [翻訳済み]

1. Uninstall deprecated packages: [翻訳済み]

   ```bash
   pnpm remove express nodemon @payloadcms/bundler-webpack @payloadcms/bundler-vite
   ```

1. Database Adapter Migrations [翻訳済み]
...
Uninstall the old package: [翻訳済み]

   ```bash
   pnpm remove @payloadcms/plugin-cloud
   ```

Install the new package: [翻訳済み]
...
1. **Optional** sharp dependency [翻訳済み]
...
To migrate, choose one of the following options: [翻訳済み]
...
_Note: for brevity, not *all* modules are listed here_ [翻訳済み]
...
1. All default view keys are now camelcase: [翻訳済み]

For example, for Root Views: [翻訳済み]
...
Email functionality has been abstracted out into email adapters. [翻訳済み]
...
### If you used the default email configuration in 2.0 (nodemailer): [翻訳済み]
...
### Removal of rate-limiting [翻訳済み]

- Now only available if using custom server and using express or similar [翻訳済み]
...
### Auth-Related Reserved Names [翻訳済み]
...
### Upload-Related Reserved Names [翻訳済み]
...
If any of these names are found in your collection / global fields, update them before migrating to avoid unexpected issues. [翻訳済み]

## Upgrade from previous beta [翻訳済み]
