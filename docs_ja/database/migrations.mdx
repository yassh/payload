...
label: マイグレーション
order: 20
keywords: database, migrations, ddl, sql, mongodb, postgres, documentation, Content Management System, cms, headless, typescript, node, react, nextjs [翻訳済み]
...
Here is an example migration file: [翻訳済み]
...
### Using database directly with the transaction [翻訳済み]
...
In SQLite, transactions are disabled by default. [More](./transactions). [翻訳済み]
...
default, migrations will be named using a timestamp. [翻訳済み]
...
Roll back the last batch of migrations. [翻訳済み]
...
Roll back all migrations that have been run, and run them again. [翻訳済み]
...
Drops all entities from the database and re-runs all migrations from scratch. [翻訳済み]

```text
npm run payload migrate:fresh
```

## When to run migrations [翻訳済み]

Depending on which Database Adapter you use, your migration workflow might differ subtly. [翻訳済み]
...
#### Postgres#postgres-migrations [翻訳済み]
...
Here is an overview of a common workflow for working locally against a development database, creating migrations, and then running migrations against your production database before deploying. [翻訳済み]

**1 - work locally using push mode** [翻訳済み]
...
For more information about push mode and prototyping in development, [click here](./postgres#prototyping-in-development-mode). [翻訳済み]
...
We won't immediately run this migration for you, however. [翻訳済み]
...
**3 - set up your build process to run migrations** [翻訳済み]
...
## Running migrations in production [翻訳済み]

In certain cases, you might want to run migrations at runtime when the server starts. Running them during build time may be impossible due to not having access to your database connection while building or similar reasoning. [翻訳済み]

If you're using a long-running server or container where your Node server starts up one time and then stays initialized, you might prefer to run migrations on server startup instead of within your CI. [翻訳済み]
...
## Environment-Specific Configurations and Migrations [翻訳済み]
...
This is an easy oversight, so be mindful of any environment-specific logic in your config when handling migrations. [翻訳済み]

**Ways to address this:** [翻訳済み]
...
- Use separate migration files for each environment to ensure the correct migration is executed in the corresponding environment. [翻訳済み]