...
keywords: overview, config, configuration, documentation, Content Management System, cms, headless, javascript, node, react, nextjs [翻訳済み]
---
...
<Banner type="success">
  **Note:** For more complex examples, see the
  [Templates](__PROTECTED_BLOCK_98__) and
  [Examples](__PROTECTED_BLOCK_99__)
...
_\* Config location detection is different between development and production environments. See below for more details._ [翻訳済み]

<Banner type="warning">
  **Important:** Ensure your __PROTECTED_BLOCK_66__ is properly configured for Payload
  to auto-detect your config location. If it does not exist, or does not specify
  the proper __PROTECTED_BLOCK_67__, Payload will default to the current working
  directory.
</Banner>

**Development Mode** [翻訳済み]
...
### Customizing the Config Location [翻訳済み]
...
<Banner type="info">
  **Tip:** __PROTECTED_BLOCK_75__ can be either an absolute path, or path
...
For more information about what we track, take a look at our [privacy policy](/privacy). [翻訳済み]

## Cross-origin resource sharing (CORS)#cors [翻訳済み]
...
Here's an example showing how to allow incoming requests from any domain: [翻訳済み]
...
Now you can run the command using: [翻訳済み]
