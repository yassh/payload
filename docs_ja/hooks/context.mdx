...
label: コンテキスト
order: 50
desc: Context allows you to pass in extra data that can be shared between hooks [翻訳済み]
keywords: hooks, context, payload context, payloadcontext, data, extra data, shared data, shared, extra [翻訳済み]
---
...
## When To Use Context [翻訳済み]

Context gives you a way forward on otherwise difficult problems such as: [翻訳済み]
...
## How To Use Context [翻訳済み]

Let's see examples on how context can be used in the first two scenarios mentioned above: [翻訳済み]

### Passing Data Between Hooks [翻訳済み]

To pass data between hooks, you can assign values to context in an earlier hook in the lifecycle of a request and expect it in the context of a later hook. [翻訳済み]
...
### Preventing Infinite Loops [翻訳済み]
