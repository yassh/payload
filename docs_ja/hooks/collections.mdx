---
title: Collection Hooks [翻訳済み]
label: コレクション
order: 20
desc: You can add hooks to any Collection, several hook types are available including beforeChange, afterRead, afterDelete and more. [翻訳済み]
keywords: hooks, collections, config, configuration, documentation, Content Management System, cms, headless, javascript, node, react, nextjs [翻訳済み]
---
Collection Hooks are [Hooks](./overview) that run on Documents within a specific [Collection](../configuration/collections). They allow you to execute your own logic during specific events of the Document lifecycle. [翻訳済み]
...
All Collection Hooks accept an array of [synchronous or asynchronous functions](./overview#async-vs-synchronous). Each Collection Hook receives specific arguments based on its own type, and has the ability to modify specific outputs. [翻訳済み]
...
Runs as the last step before documents are returned. Flattens locales, hides protected fields, and removes fields that users do not have access to. [翻訳済み]
