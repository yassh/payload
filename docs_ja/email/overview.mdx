---
title: Email Functionality [翻訳済み]
label: 概要
order: 10
...
keywords: email, overview, config, configuration, documentation, Content Management System, cms, headless, javascript, node, react, nextjs [翻訳済み]
---
## Introduction [翻訳済み]
...
An email adapter will require at least the following fields: [翻訳済み]
...
**Example email options using SMTP:** [翻訳済み]
...
**Example email options using nodemailer.createTransport:** [翻訳済み]
...
**Custom Transport:** [翻訳済み]

You also have the ability to bring your own nodemailer transport. This is an example of using the SendGrid nodemailer transport. [翻訳済み]
...
This will log the ethereal.email details to console on startup. [翻訳済み]
...
## Resend Configuration [翻訳済み]

The Resend adapter requires an API key to be passed in the options. This can be found in the Resend dashboard. This is the preferred package if you are deploying on Vercel because this is much more lightweight than the Nodemailer adapter. [翻訳済み]

| Option | Description                         |
| ------ | ----------------------------------- |
| apiKey | The API key for the Resend service. | [翻訳済み]
...
## Using multiple mail providers [翻訳済み]
