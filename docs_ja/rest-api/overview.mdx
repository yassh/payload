...
keywords: rest, api, documentation, Content Management System, cms, headless, javascript, node, react, nextjs [翻訳済み]
---
...
- [joins](/docs/fields/join#rest-api) - specifies the custom request for each join field by name of the field [翻訳済み]
...
Note: Collection slugs must be formatted in kebab-case [翻訳済み]

**All CRUD operations are exposed as follows:** [翻訳済み]
  ...
  data={[
    {
      ...
      example: {
        ...
        drawerContent: __PROTECTED_BLOCK_11__find\__PROTECTED_BLOCK_12__find\__PROTECTED_BLOCK_13__
  },
  },
  {
...
Additional REST API endpoints can be added to your application by providing an array of __PROTECTED_BLOCK_14__ in various places within a Payload Config. Custom endpoints are useful for adding additional middleware on existing routes or for building custom functionality into Payload apps and plugins. Endpoints can be added at the top of the Payload Config, __PROTECTED_BLOCK_15__, and __PROTECTED_BLOCK_16__ and accessed respective of the api and slugs you have configured.
...
Each endpoint object needs to have: [翻訳済み]
...
Or you could use our helper function that mutates the request and appends data and file if found. [翻訳済み]
...
The locale and the fallback locale are not automatically appended to custom endpoint requests. If you would like to add them you can use this helper function. [翻訳済み]
...
## Method Override for GET Requests [翻訳済み]
...
Here is an example of how to use the method override to perform a GET request: [翻訳済み]

#### Using Method Override (POST) [翻訳済み]
...
#### Equivalent Regular GET Request [翻訳済み]
