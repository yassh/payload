---
title: Document Locking [翻訳済み]
label: Document Locking [翻訳済み]
order: 40
desc: Ensure your documents are locked during editing to prevent concurrent changes from multiple users and maintain data integrity. [翻訳済み]
...
The lock is automatically triggered when a user begins editing a document within the Admin Panel and remains in place until the user exits the editing view or the lock expires due to inactivity. [翻訳済み]
...
- Return to Dashboard: Navigate away from the locked document and continue with other tasks. [翻訳済み]
...
<Banner type="info">
  **Note:** If your application does not require document locking, you can
  ...
  __PROTECTED_BLOCK_4__ property to __PROTECTED_BLOCK_5__.
</Banner>
...
Here's an example configuration for document locking: [翻訳済み]
...
Document locking affects both the Local and REST APIs, ensuring that if a document is locked, concurrent users will not be able to perform updates or deletes on that document (including globals). If a user attempts to update or delete a locked document, they will receive an error. [翻訳済み]

Once the document is unlocked or the lock duration has expired, other users can proceed with updates or deletes as normal. [翻訳済み]

#### Overriding Locks [翻訳済み]
...
This option is particularly useful in scenarios where administrative privileges or specific workflows require you to override the lock and ensure the operation is completed. [翻訳済み]