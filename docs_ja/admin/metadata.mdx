---
title: Page Metadata [翻訳済み]
label: メタデータ
order: 70
desc: Customize the metadata of your pages within the Admin Panel [翻訳済み]
keywords: admin, components, custom, documentation, Content Management System, cms, headless, javascript, node, react, nextjs [翻訳済み]
---
Every page within the Admin Panel automatically receives dynamic, auto-generated metadata derived from live document data, the user's current locale, and more. This includes the page title, description, og:image, etc. and requires no additional configuration. [翻訳済み]

Metadata is fully configurable at the root level and cascades down to individual collections, documents, and custom views. This allows for the ability to control metadata on any page with high precision, while also providing sensible defaults. [翻訳済み]
...
Within the Admin Panel, metadata can be customized at the following levels: [翻訳済み]

- [Root Metadata](#root-metadata)
- [Collection Metadata](#collection-metadata)
- [Global Metadata](#global-metadata)
- [View Metadata](#view-metadata) [翻訳済み]
...
Root Metadata is the metadata that is applied to all pages within the Admin Panel. This is where you can control things like the suffix appended onto each page's title, the favicon displayed in the browser's tab, and the Open Graph data that is used when sharing the Admin Panel on social media. [翻訳済み]
...
The following options are available for Root Metadata: [翻訳済み]
...
By default, the Admin Panel is set to prevent search engines from indexing pages within the Admin Panel. [翻訳済み]
...
<Banner type="info">
  **Note:** If you've customized the path to your Admin Panel via
  __PROTECTED_BLOCK_39__, be sure to update the __PROTECTED_BLOCK_40__ directive to match your
  custom path.
</Banner>

## Collection Metadata [翻訳済み]

Collection Metadata is the metadata that is applied to all pages within any given Collection within the Admin Panel. This metadata is used to customize the title and description of all views within any given Collection, unless overridden by the view itself. [翻訳済み]
...
The Collection Meta config has the same options as the [Root Metadata](#root-metadata) config. [翻訳済み]

## Global Metadata [翻訳済み]

Global Metadata is the metadata that is applied to all pages within any given Global within the Admin Panel. This metadata is used to customize the title and description of all views within any given Global, unless overridden by the view itself. [翻訳済み]
...
The Global Meta config has the same options as the [Root Metadata](#root-metadata) config. [翻訳済み]
