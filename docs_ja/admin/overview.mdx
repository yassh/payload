...
keywords: admin, components, custom, customize, documentation, Content Management System, cms, headless, javascript, node, react, nextjs [翻訳済み]
---
...
<LightDarkImage
  srcLight="__PROTECTED_BLOCK_102__
  srcDark="__PROTECTED_BLOCK_103__
...
The Admin Panel serves as the entire HTTP layer for Payload, providing a full CRUD interface for your app. This means that both the [REST](../rest-api/overview) and [GraphQL](../graphql/overview) APIs are simply [Next.js Routes](__PROTECTED_BLOCK_104__) that exist directly alongside your front-end application.
...
<Banner type="info">
  ...
  about it here](__PROTECTED_BLOCK_105__).
</Banner>
...
All auto-generated files will contain the following comments at the top of each file: [翻訳済み]
...
<Banner type="success">
  **Reminder:** These are the _root-level_ options for the Admin Panel. You can
  ...
  __PROTECTED_BLOCK_38__ keys.
</Banner>

### The Admin User Collection [翻訳済み]
...
The Admin Panel can only be used by a single auth-enabled Collection. To enable authentication for a Collection, simply set __PROTECTED_BLOCK_40__ in the Collection's configuration. See [Authentication](../authentication/overview) for more information.
...
### Role-based Access Control [翻訳済み]
...
#### Customizing Root-level Routes [翻訳済み]

You can change the Root-level Routes as needed, such as to mount the Admin Panel at the root of your application. [翻訳済み]
...
<Banner type="warning">
  **Note:** If you set Root-level Routes _before_ auto-generating the Admin
  Panel via __PROTECTED_BLOCK_69__, your [Project Structure](#project-structure)
  will already be set up correctly.
</Banner>

### Admin-level Routes [翻訳済み]
...
<Banner type="success">
  **Note:** You can also swap out entire _views_ out for your own, using the
  __PROTECTED_BLOCK_88__ property of the Payload Config. See [Custom
...
<Banner type="info">
  **Important** You must enable timezones on each individual date field via
  __PROTECTED_BLOCK_96__. See [Date Fields](../fields/overview#date) for more
  information.
</Banner>
