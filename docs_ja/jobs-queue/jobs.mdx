...
label: ジョブ
order: 40
desc: A Job is a set of work that is offloaded from your APIs and will be processed at a later date. [翻訳済み]
keywords: jobs queue, application framework, typescript, node, react, nextjs [翻訳済み]
---
Now that we have covered Tasks and Workflows, we can tie them together with a concept called a Job. [翻訳済み]
...
In addition to being able to queue new Jobs based on Workflows, you can also queue a job for a single Task: [翻訳済み]
