...
label: キュー
order: 50
desc: A Queue is a specific group of jobs which can be executed in the order that they were added. [翻訳済み]
...
**But, imagine if you wanted to have some jobs that run nightly, and other jobs which should run every five minutes.** [翻訳済み]
...
Then, you could configure two different runner strategies: [翻訳済み]
...
As mentioned above, you can queue jobs, but the jobs won't run unless a worker picks up your jobs and runs them. This can be done in four ways: [翻訳済み]
...
This endpoint is automatically mounted for you and is helpful in conjunction with serverless platforms like Vercel, where you might want to use Vercel Cron to invoke a serverless function that executes your jobs. [翻訳済み]

**Vercel Cron Example** [翻訳済み]
...
Here's an example of what this file will look like: [翻訳済み]
...
If you want to process jobs programmatically from your server-side code, you can use the Local API: [翻訳済み]
...
## Processing Order [翻訳済み]

By default, jobs are processed first in, first out (FIFO). This means that the first job added to the queue will be the first one processed. However, you can also configure the order in which jobs are processed. [翻訳済み]

### Jobs Configuration [翻訳済み]
...
You can also set this on a queue-by-queue basis: [翻訳済み]
...
If you need even more control over the processing order, you can pass a function that returns the processing order - this function will be called every time a queue starts processing jobs. [翻訳済み]
