---
title: Respecting Access Control with Local API Operations [翻訳済み]
label: アクセス制御
order: 40
...
### Default Behavior: Access Control Skipped [翻訳済み]

By default, **local API operations skip access control**. This allows operations to execute without the system checking if the current user has appropriate permissions. This might be helpful in admin or server-side scripts where the user context is not required to perform the operation. [翻訳済み]
...
### Respecting Access Control [翻訳済み]
