---
title: Slate Editor [翻訳済み]
label: Slate (legacy) [翻訳済み]
order: 100
...
keywords: slatejs, slate, rich text, editor, headless cms [翻訳済み]
---
...
To use the Slate editor, first you need to install it: [翻訳済み]
...
And here's an example for how to install the Slate editor on a field-by-field basis, while customizing its options: [翻訳済み]
...
This allows [fields](/docs/fields/overview) to be saved as extra fields on a link inside the Rich Text Editor. When this is present, the fields will render inside a modal that can be opened by clicking the "edit" button on the link element. [翻訳済み]
...
This allows [fields](/docs/fields/overview) to be saved as meta data on an upload field inside the Rich Text Editor. When this is present, the fields will render inside a modal that can be opened by clicking the "edit" button on the upload element. [翻訳済み]
...
### Specifying which elements and leaves to allow [翻訳済み]

To specify which default elements or leaves should be allowed to be used for this field, define arrays that contain string names for each element or leaf you wish to enable. To specify a custom element or leaf, pass an object with all corresponding properties as outlined below. View the [example](#example) to reference how this all works. [翻訳済み]

### Building custom elements and leaves [翻訳済み]
...
Once you're up to speed with the general concepts involved, you can pass in your own elements and leaves to your field's admin config. [翻訳済み]

**Both custom elements and leaves are defined via the following config:** [翻訳済み]
...
### Generating HTML [翻訳済み]

As the Rich Text field saves its content in a JSON format, you'll need to render it as HTML yourself. Here is an example for how to generate JSX / HTML from Rich Text content: [翻訳済み]
