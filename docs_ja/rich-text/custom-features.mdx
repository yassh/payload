...
label: カスタム機能
order: 40
desc: Building custom features [翻訳済み]
keywords: lexical, rich text, editor, headless cms, feature, features [翻訳済み]
---
...
Lexical features are designed to be modular, meaning each piece of functionality is encapsulated within just two specific interfaces: one for server-side code and one for client-side code. [翻訳済み]
...
Using the BlocksFeature, you can add both inline blocks (= can be inserted into a paragraph, in between text) and block blocks (= take up the whole line) to the editor. If you simply want to bring custom react components into the editor, this is the way to go. [翻訳済み]

### Example: Code Field Block with language picker [翻訳済み]
...
Custom Blocks are not enough? To start building a custom feature, you should start with the server feature, which is the entry-point. [翻訳済み]

**Example myFeature/feature.server.ts:** [翻訳済み]
...
Now, the feature is ready to be used in the editor: [翻訳済み]
...
By default, this server feature does nothing - you haven't added any functionality yet. Depending on what you want your feature to do, the ServerFeature type exposes various properties you can set to inject custom functionality into the lexical editor. [翻訳済み]

### i18n [翻訳済み]

Each feature can register their own translations, which are automatically scoped to the feature key: [翻訳済み]
...
### Markdown Transformers#server-feature-markdown-transformers [翻訳済み]

The Server Feature, just like the Client Feature, allows you to add markdown transformers. Markdown transformers on the server are used when [converting the editor from or to markdown](/docs/rich-text/converting-markdown). [翻訳済み]
...
### Nodes#server-feature-nodes [翻訳済み]

While nodes added to the server feature do not control how the node is rendered in the editor, they control other aspects of the node: [翻訳済み]

- HTML conversion
- Node Hooks
- Sub fields
- Behavior in a headless editor [翻訳済み]
...
While nodes in the client feature are added by themselves to the nodes array, nodes in the server feature can be added together with the following sibling options: [翻訳済み]
...
To set up your client-side feature, follow these three steps: [翻訳済み]
...
**Example myFeature/feature.client.ts:** [翻訳済み]
...
### Adding a client feature to the server feature [翻訳済み]
...
### Nodes#client-feature-nodes [翻訳済み]
...
**myFeature/feature.client.ts:** [翻訳済み]
...
**myFeature/nodes/MyNode.tsx:** [翻訳済み]

Here is a basic DecoratorNode example: [翻訳済み]
...
Please do not add any 'use client' directives to your nodes, as the node class can be used on the server. [翻訳済み]
...
In this example, we register a lexical command, which simply inserts a new MyNode into the editor. This command can be called from anywhere within lexical, e.g. from within a custom node. [翻訳済み]
...
Within dropdown groups, items are positioned vertically when the dropdown is opened and include the icon & label. Within button groups, items are positioned horizontally and only include the icon. If a toolbar group with the same key is declared twice, all its items will be merged into one group. [翻訳済み]

#### Custom buttons toolbar group [翻訳済み]
...
#### Custom dropdown toolbar group [翻訳済み]
...
Custom nodes and features on its own are pointless, if they can't be added to the editor. You will need to hook in one of our interfaces which allow the user to interact with the editor: [翻訳済み]
...
You will have to provide a toolbar group first, and then the items for that toolbar group (more on that above). [翻訳済み]
...
By creating a helper function like this, you can easily re-use it and add items to it. All Slash Menu groups with the same keys will have their items merged together. [翻訳済み]
...
### Slash Menu items [翻訳済み]

The API for adding items to the slash menu is similar. There are slash menu groups, and each slash menu groups has items. Here is an example: [翻訳済み]
...
### Markdown Transformers#client-feature-markdown-transformers [翻訳済み]

The Client Feature, just like the Server Feature, allows you to add markdown transformers. Markdown transformers on the client are used to create new nodes when a certain markdown pattern is typed in the editor. [翻訳済み]
...
To accept props in your feature, type them as a generic. [翻訳済み]
...
In the client feature, it works similarly: [翻訳済み]
...
### Bringing props from the server to the client [翻訳済み]

By default, the client feature will never receive any props from the server feature. In order to pass props from the server to the client, you can need to return those props in the server feature: [翻訳済み]
