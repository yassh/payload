...
label: 下書き
order: 20
desc: Enable drafts on collection documents or globals and build true preview environments for your data. [翻訳済み]
keywords: version history, drafts, preview, draft, restore, publish, autosave, Content Management System, cms, headless, javascript, node, react, nextjs [翻訳済み]
---
...
By enabling Versions with Drafts, your collections and globals can maintain _newer_, and _unpublished_ versions of your documents. It's perfect for cases where you might want to work on a document, update it and save your progress, but not necessarily make it publicly published right away. Drafts are extremely helpful when building preview implementations. [翻訳済み]

![Drafts Enabled](/images/docs/drafts-enabled.png)
_If Drafts are enabled, the typical Save button is replaced with new actions which allow you to either save a draft, or publish your changes._ [翻訳済み]
...
**Admin UI status indication** [翻訳済み]

Within the Admin UI, if drafts are enabled, a document can be shown with one of three "statuses": [翻訳済み]
...
#### Updating or creating drafts [翻訳済み]
...
#### Reading drafts vs. published documents [翻訳済み]
...
**For example, let's take the following scenario:** [翻訳済み]
...
1. You then make some further updates, and save more updates as another draft [翻訳済み]
...
<Banner type="error">
  **Important:** the __PROTECTED_BLOCK_40__ argument on its own will not restrict documents
  with __PROTECTED_BLOCK_41__ from being returned from the API. You need to use
  Access Control to prevent documents with __PROTECTED_BLOCK_42__ from being
  returned to unauthenticated users. Read below for more information on how this
  works.
</Banner>

## Controlling who can see Collection drafts [翻訳済み]
...
_will not have a __PROTECTED_BLOCK_45__ field_ until you resave them. For this reason, if you are
...
**__PROTECTED_BLOCK_46__ is equal to __PROTECTED_BLOCK_47__** as well as where
**__PROTECTED_BLOCK_48__ does not exist**.
