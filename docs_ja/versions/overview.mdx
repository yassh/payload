...
label: 概要
order: 10
desc: Keep a version history or audit log of changes and publish collection documents and globals. [翻訳済み]
keywords: version history, revisions, audit log, draft, publish, restore, autosave, Content Management System, cms, headless, javascript, node, react, nextjs [翻訳済み]
---
...
_Comparing an old version to a newer version of a document_ [翻訳済み]

**With Versions, you can:** [翻訳済み]
...
- Build a powerful publishing schedule mechanism to create documents and have them become publicly readable automatically at a future date [翻訳済み]
...
Versions support a few different levels of functionality that each come with their own impacts to document workflow. [翻訳済み]

### Versions enabled, drafts disabled [翻訳済み]
...
For example, a use case for "versions enabled, drafts disabled" could be on a collection of users, where you might want to keep a version history (or audit log) of all changes ever made to users - but any changes to users should _always_ be treated as "published" and you have no need to maintain a "draft" version of a user. [翻訳済み]

### Versions and drafts enabled [翻訳済み]
...
### Versions, drafts, and autosave enabled [翻訳済み]
...
Global versions work similarly to Collection versions but have a slightly different set of config properties supported. [翻訳済み]
...
**Collection REST endpoints:** [翻訳済み]
...
**Collection Local API methods:** [翻訳済み]
...
Versions expose a new [Access Control](../access-control/overview) function on both [Collections](../configuration/collections) and [Globals](../configuration/globals) that allow for you to control who can see versions of documents, and who can't. [翻訳済み]
...
For full details on how to use Access Control with Versions, see the [Access Control](../access-control/overview) documentation. [翻訳済み]