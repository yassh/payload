---
title: API Key Strategy [翻訳済み]
label: API Key Strategy [翻訳済み]
order: 50
...
1. Generate a non-expiring API key for that user to request with. [翻訳済み]
...
Technically, both of these options will work for third-party integrations but the second option with API key is simpler, because it reduces the amount of work that your integrations need to do to be authenticated properly. [翻訳済み]
...
your API keys will not be. [翻訳済み]

<Banner type="warning">
  **Important:**
  If you change your __PROTECTED_BLOCK_6__, you will need to regenerate your API keys.
