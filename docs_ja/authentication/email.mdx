---
title: Authentication Emails [翻訳済み]
label: Email Verification [翻訳済み]
order: 30
...
keywords: authentication, email, config, configuration, documentation, Content Management System, cms, headless, javascript, node, react, nextjs [翻訳済み]
---
...
## Email Verification [翻訳済み]

Email Verification forces users to prove they have access to the email address they can authenticate. This will help to reduce spam accounts and ensure that users are who they say they are. [翻訳済み]
...
This function allows for overriding the HTML within emails that are sent to users attempting to reset their password. The function should return a string that supports HTML, which can be a full HTML email. [翻訳済み]
