---
title: Authentication Operations [翻訳済み]
label: 操作
order: 20
desc: Enabling Authentication automatically makes key operations available such as Login, Logout, Verify, Unlock, Reset Password and more. [翻訳済み]
keywords: authentication, config, configuration, documentation, Content Management System, cms, headless, javascript, node, react, nextjs [翻訳済み]
---
...
Returns either a logged in user with token or null when there is no logged in user. [翻訳済み]
...
<Banner type="success">
  **Server Functions:** Payload offers a ready-to-use __PROTECTED_BLOCK_29__ server function
...
<Banner type="success">
  **Server Functions:** Payload provides a ready-to-use __PROTECTED_BLOCK_31__ server function
...
This operation requires a non-expired token to send back a new one. If the user's token has already expired, you will need to allow them to log in again to retrieve a new token. [翻訳済み]

If successful, this operation will automatically renew the user's HTTP-only cookie and will send back the updated token in JSON. [翻訳済み]

**Example REST API token refresh**: [翻訳済み]
...
<Banner type="success">
  **Server Functions:** Payload exports a ready-to-use __PROTECTED_BLOCK_33__ server function
...
## Verify by Email [翻訳済み]

If your collection supports email verification, the Verify operation will be exposed which accepts a verification token and sets the user's `_verified` property to `true`, thereby allowing the user to authenticate with the Payload API.

**Example REST API user verification**: [翻訳済み]
...
**Example Local API verification**: [翻訳済み]
...
If a user locks themselves out and you wish to deliberately unlock them, you can utilize the Unlock operation. The [Admin Panel](../admin/overview) features an Unlock control automatically for all collections that feature max login attempts, but you can programmatically unlock users as well by using the Unlock operation. [翻訳済み]
...
The link to reset the user's password contains a token which is what allows the user to securely reset their password. [翻訳済み]

By default, the Forgot Password operations send users to the [Admin Panel](../admin/overview) to reset their password, but you can customize the generated email to send users to the frontend of your app instead by [overriding the email HTML](/docs/authentication/email#forgot-password). [翻訳済み]

**Example REST API Forgot Password**: [翻訳済み]
...
**Example Local API forgot password**: [翻訳済み]
...
<Banner type="info">
  **Note:** if you do not have a __PROTECTED_BLOCK_48__ set, Payload will attempt to
  create one for you if the __PROTECTED_BLOCK_49__ operation was triggered via REST
  or GraphQL by looking at the incoming __PROTECTED_BLOCK_50__. But this is not supported if you
  are calling __PROTECTED_BLOCK_51__ via the Local API. If you do not have a
  __PROTECTED_BLOCK_52__ set, you may want to override your
  __PROTECTED_BLOCK_53__ function to provide a full URL to link
...
After a user has "forgotten" their password and a token is generated, that token can be used to send to the reset password operation along with a new password which will allow the user to reset their password securely. [翻訳済み]

**Example REST API Reset Password**: [翻訳済み]
