---
title: Cookie Strategy [翻訳済み]
label: Cookie Strategy [翻訳済み]
order: 40
...
<Banner type="success">
  **Tip:** You can access the logged-in user from within [Access
  ...
  the __PROTECTED_BLOCK_9__ argument. [More details](./token-data).
</Banner>

### Automatic browser inclusion [翻訳済み]
...
Fetch example, including credentials: [翻訳済み]
...
In this scenario, if your cookie was still valid, malicious-intent.com would be able to make requests like the one above on your behalf. This is a CSRF attack. [翻訳済み]

### CSRF Prevention [翻訳済み]
...
#### Cross domain authentication [翻訳済み]
...
##### 1. Use subdomains [翻訳済み]

Cookies can cross subdomains without being considered third party cookies, for example if your API is at api.example.com then you can authenticate from example.com. [翻訳済み]

##### 2. Configure cookies [翻訳済み]

If option 1 isn't possible, then you can get around this limitation by [configuring your cookies](./overview#config-options) on your authentication collection to achieve the following setup: [翻訳済み]
...
<Banner type="success">
  **Good to know:** Setting up __PROTECTED_BLOCK_17__ will not work if you're developing
  on __PROTECTED_BLOCK_18__ or any non-https domain. For local development you
  should conditionally set this to __PROTECTED_BLOCK_19__ based on the environment.
</Banner>
