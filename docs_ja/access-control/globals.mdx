---
title: Globals Access Control [翻訳済み]
label: グローバル
order: 30
desc: Global-level Access Control is specified within each Global's `access` property and allows you to define which users can read or update Globals. [翻訳済み]
keywords: globals, access control, permissions, documentation, Content Management System, cms, headless, javascript, node, react, nextjs [翻訳済み]
---
Global Access Control is [Access Control](../access-control/overview) used to restrict access to [Global](../configuration/globals) Documents, as well as what they can and cannot see within the [Admin Panel](../admin/overview) as it relates to that Global. [翻訳済み]
...
If a Global supports [Versions](../versions/overview), the following additional options are available: [翻訳済み]
...
Returns a boolean result or optionally a [query constraint](../queries/overview) which limits who can read this global based on its current properties. [翻訳済み]
...
Returns a boolean result or optionally a [query constraint](../queries/overview) which limits who can update this global based on its current properties. [翻訳済み]
