...
## Quickstart with create-payload-app [翻訳済み]
...
## Adding to an existing app [翻訳済み]
...
#### 1. Install the relevant packages [翻訳済み]
...
<Banner type="warning">
  **Note:** Swap out __PROTECTED_BLOCK_11__ for your package manager. If you are using npm, you
  might need to install using legacy peer deps: __PROTECTED_BLOCK_12__.
</Banner>
...
To install a Database Adapter, you can run **one** of the following commands: [翻訳済み]
...
- To install the [Postgres Adapter](../database/postgres), run: [翻訳済み]
...
<Banner type="warning">
  ...
  Group](__PROTECTED_BLOCK_45__),
  i.e. __PROTECTED_BLOCK_16__.
</Banner>
...
<Banner type="warning">
  **Important:** Payload is a fully ESM project, and that means the
  __PROTECTED_BLOCK_26__ function is an ECMAScript module.
</Banner>

To import the Payload Plugin, you need to make sure your `next.config` file is set up to use ESM.

You can do this in one of two ways: [翻訳済み]
...
Although this is just the bare minimum config, there are _many_ more options that you can control here. To reference the full config and all of its options, [click here](/docs/configuration/overview). [翻訳済み]
