...
keywords: query, documents, pagination, documentation, Content Management System, cms, headless, javascript, node, react, nextjs [翻訳済み]
---
...
<Banner type="warning">
  **Important:** To perform querying with __PROTECTED_BLOCK_9__ efficiently, Payload
  implements your __PROTECTED_BLOCK_10__ query on the database level. Because of that, your
  __PROTECTED_BLOCK_11__ and __PROTECTED_BLOCK_12__ hooks may not receive the full __PROTECTED_BLOCK_13__. To ensure
  that some fields are always selected for your hooks / access control,
  regardless of the __PROTECTED_BLOCK_14__ query you can use __PROTECTED_BLOCK_15__ collection config
  property.
</Banner>
...
To understand the syntax, you need to understand that complex URL search strings are parsed into a JSON object. This one isn't too bad, but more complex queries get unavoidably more difficult to write. [翻訳済み]
...
<Banner type="info">
  **Reminder:** This is the same for [Globals](../configuration/globals) using
  the __PROTECTED_BLOCK_18__ endpoint.
</Banner>

## defaultPopulate collection config property [翻訳済み]
...
With this feature, you can dramatically reduce the amount of JSON that is populated from [Relationship](/docs/fields/relationship) or [Upload](/docs/fields/upload) fields. [翻訳済み]
...
<Banner type="warning">
  **Important:** When using __PROTECTED_BLOCK_26__ on a collection with
  [Uploads](/docs/fields/upload) enabled and you want to select the __PROTECTED_BLOCK_27__ field,
  it is important to specify __PROTECTED_BLOCK_28__ as well, otherwise Payload will
  not be able to construct the correct file URL, instead returning __PROTECTED_BLOCK_29__.
</Banner>
