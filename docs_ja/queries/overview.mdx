---
title: Querying your Documents [翻訳済み]
label: 概要
order: 10
...
keywords: query, documents, overview, documentation, Content Management System, cms, headless, javascript, node, react, nextjs [翻訳済み]
---
...
To query your Documents, you can send any number of [Operators](#operators) through your request: [翻訳済み]
...
_The exact query syntax will depend on the API you are using, but the concepts are the same across all APIs. [More details](#writing-queries)._ [翻訳済み]
...
The following operators are available for use in queries: [翻訳済み]
...
<Banner type="success">
  **Tip:** If you know your users will be querying on certain fields a lot, add
  __PROTECTED_BLOCK_24__ to the Field Config. This will speed up searches using that
  field immensely.
</Banner>

### And / Or Logic [翻訳済み]

In addition to defining simple queries, you can join multiple queries together using AND / OR logic. These can be nested as deeply as you need to create complex queries. [翻訳済み]
