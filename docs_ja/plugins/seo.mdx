...
keywords: plugins, seo, meta, search, engine, ranking, google [翻訳済み]
label: SEO
order: 30
title: SEO Plugin [翻訳済み]
---
...
<Banner type="info">
  ...
  here](__PROTECTED_BLOCK_75__). If
  you need help, check out our [Community
  Help](__PROTECTED_BLOCK_76__). If you think you've found a bug,
  please [open a new
  issue](__PROTECTED_BLOCK_77__)
...
A function that takes in the default fields via an object and expects an array of fields in return. You can use this to modify existing fields or add new ones. [翻訳済み]
...
<Banner type="info">
  If you wish to continue to use top-level or sidebar fields with __PROTECTED_BLOCK_41__,
  you must not let the default __PROTECTED_BLOCK_42__ tab get created for you (see the note
  above). Instead, you must define the first field of your config with type
  __PROTECTED_BLOCK_43__ and place all other fields adjacent to this one.
</Banner>

##### `generateTitle`

A function that allows you to return any meta title, including from the document's content. [翻訳済み]
...
All "generate" functions receive the following arguments: [翻訳済み]
...
A function that allows you to return any meta description, including from the document's content. [翻訳済み]
...
A function that allows you to return any meta image, including from the document's content. [翻訳済み]
...
A function called by the search preview component to display the actual URL of your page. [翻訳済み]
...
<Banner type="info">
  ...
  __PROTECTED_BLOCK_72__ and __PROTECTED_BLOCK_73__ to override the length rules.
</Banner>
