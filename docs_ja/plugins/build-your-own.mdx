---
title: Building Your Own Plugin [翻訳済み]
label: 独自プラグインの作成
order: 20
...
keywords: plugins, template, config, configuration, extensions, custom, documentation, Content Management System, cms, headless, javascript, node, react, nextjs [翻訳済み]
---
...
<Banner type="success">
  To use the template, run __PROTECTED_BLOCK_11__
  directly in your terminal.
</Banner>

Our plugin template includes everything you need to build a full life-cycle plugin: [翻訳済み]
...
By abstracting your code into a plugin, you'll be able to reuse your feature across multiple projects and make it available for other developers to use. [翻訳済み]
...
The initialization process goes in the following order: [翻訳済み]

1. Incoming config is validated
2. Plugins execute
3. Default options are integrated
4. Sanitization cleans and validates data
5. Final config gets initialized [翻訳済み]

## Plugin Template [翻訳済み]
...
- **package**.json - Contains necessary scripts and dependencies. Overwrite the metadata in this file to describe your Plugin. [翻訳済み]
...
Do **not** store any of the plugin functionality in this folder - it is purely an environment to _assist_ you with developing the plugin. [翻訳済み]

If you're starting from scratch, you can easily setup a dev environment like this: [翻訳済み]
...
Another benefit of the dev folder is that you have the perfect environment established for testing. [翻訳済み]
...
For development and testing, you will likely need some data to work with. You can streamline this process by seeding and dropping your database - instead of manually entering data. [翻訳済み]
...
## Building a Plugin [翻訳済み]

Now that we have our environment setup and dev project ready to go - it's time to build the plugin! [翻訳済み]
...
Let's say you want to build a plugin that adds a new collection: [翻訳済み]
...
This same logic is applied to other array and object like properties such as admin, globals and hooks: [翻訳済み]
...
### Extending functions [翻訳済み]

Function properties cannot use spread syntax. The way to extend them is to execute the existing function if it exists and then run your additional functionality. [翻訳済み]
...
In addition to the setup covered above, here are other best practices to follow: [翻訳済み]

### Providing an enable / disable option [翻訳済み]

For a better user experience, provide a way to disable the plugin without uninstalling it. [翻訳済み]

### Include tests in your GitHub CI workflow [翻訳済み]
...
### Add payload-plugin topic tag [翻訳済み]
...
### Use Semantic Versioning (SemVer) [翻訳済み]
