...
keywords: uploads, images, media, overview, documentation, Content Management System, cms, headless, javascript, node, react, nextjs [翻訳済み]
---
...
<LightDarkImage
  srcLight="__PROTECTED_BLOCK_172__
  srcDark="__PROTECTED_BLOCK_173__
...
1. __PROTECTED_BLOCK_10__, __PROTECTED_BLOCK_11__, and __PROTECTED_BLOCK_12__ fields will be automatically added to your Collection. Optionally, if you pass __PROTECTED_BLOCK_13__ to your Collection's Upload config, a [__PROTECTED_BLOCK_14__](#image-sizes) array will also be added containing auto-resized image sizes and filenames.
1. The Admin Panel will modify its built-in __PROTECTED_BLOCK_15__ component to show a thumbnail for each upload within the List View
1. The Admin Panel will modify its __PROTECTED_BLOCK_16__ view(s) to add a new set of corresponding Upload UI which will allow for file upload
1. The __PROTECTED_BLOCK_17__, __PROTECTED_BLOCK_18__, and __PROTECTED_BLOCK_19__ Collection operations will be modified to support file upload, re-upload, and deletion

## Enabling Uploads

Every Payload Collection can opt-in to supporting Uploads by specifying the __PROTECTED_BLOCK_20__ property on the Collection's config to either __PROTECTED_BLOCK_21__ or to an object containing __PROTECTED_BLOCK_22__ options.
...
### Collection Upload Options [翻訳済み]

_An asterisk denotes that an option is required._ [翻訳済み]
...
### Custom filename via hooks [翻訳済み]
...
The [Admin Panel](../admin/overview) will also automatically display all available files, including width, height, and file size, for each of your uploaded files. [翻訳済み]
...
#### Accessing the resized images in hooks [翻訳済み]
...
The object will have keys for each size generated, and each key will be set equal to a buffer containing the file data. [翻訳済み]

#### Handling Image Enlargement [翻訳済み]

When an uploaded image is smaller than the defined image size, we have 3 options: [翻訳済み]
...
image size. Use the __PROTECTED_BLOCK_119__ prop to change this.

</Banner>

#### Custom file name per size [翻訳済み]
...
## Crop and Focal Point Selector [翻訳済み]

This feature is only available for image file types. [翻訳済み]
...
Image cropping occurs before any resizing, the resized images will therefore be generated from the cropped image (**not** the original image). [翻訳済み]

If no resizing options are specified (`imageSizes` or `resizeOptions`), the focal point selector will not be displayed.

## Disabling Local Upload Storage [翻訳済み]
...
<Banner type="info">
  **Note:** To include any additional fields (like __PROTECTED_BLOCK_138__, __PROTECTED_BLOCK_139__, etc.), append
  a __PROTECTED_BLOCK_140__ field containing a JSON-stringified object of the required
...
## Uploading Files stored locally [翻訳済み]
...
Remember that all custom hooks attached to the __PROTECTED_BLOCK_145__ collection will still trigger.
Ensure that files match the specified mimeTypes or sizes defined in the collection's __PROTECTED_BLOCK_146__ or custom __PROTECTED_BLOCK_147__.

</Banner>

## Uploading Files from Remote URLs [翻訳済み]
...
#### Configuration Example [翻訳済み]

Here’s how to configure the pasteURL option to control remote URL fetching: [翻訳済み]
