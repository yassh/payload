---
title: Email Field [翻訳済み]
label: メール
order: 80
desc: The Email field enforces that the value provided is a valid email address. Learn how to use Email fields, see examples and options. [翻訳済み]
keywords: email, fields, config, configuration, documentation, Content Management System, cms, headless, javascript, node, react, nextjs [翻訳済み]
---
The Email Field enforces that the value provided is a valid email address. [翻訳済み]

<LightDarkImage
  srcLight="__PROTECTED_BLOCK_31__
  srcDark="__PROTECTED_BLOCK_32__
...
The Email Field inherits all of the default admin options from the base [Field Admin Config](./overview#admin-options), plus the following additional options: [翻訳済み]
