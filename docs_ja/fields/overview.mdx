...
keywords: overview, fields, config, configuration, documentation, Content Management System, cms, headless, javascript, node, react, nextjs [翻訳済み]
label: 概要
order: 10
title: Fields Overview [翻訳済み]
---
...
There are many [Field Types](#field-types) to choose from, ranging anywhere from simple text strings to nested arrays and blocks. Most fields save data to the database, while others are strictly presentational. Fields can have [Custom Validations](#validation), [Conditional Logic](./overview#conditional-logic), [Access Control](#field-level-access-control), [Hooks](#field-level-hooks), and so much more. [翻訳済み]

Fields can be endlessly customized in their appearance and behavior without affecting their underlying data structure. Fields are designed to withstand heavy modification or even complete replacement through the use of [Custom Field Components](#custom-components). [翻訳済み]
...
<Banner type="warning">
  **Reminder:** Each field is an object with at least the __PROTECTED_BLOCK_36__ property. This
...
- [Virtual Fields](#virtual-fields) [翻訳済み]

To begin writing fields, first determine which [Field Type](#field-types) best supports your application. Then author your field accordingly using the [Field Options](#field-options) for your chosen field type. [翻訳済み]
...
Here are the available Data Fields: [翻訳済み]
...
- [Upload](./upload) - allows local file and image upload [翻訳済み]

### Presentational Fields [翻訳済み]

Presentational Fields do not store data in the database. Instead, they are used to organize and present other fields in the [Admin Panel](../admin/overview), or to add custom UI components. [翻訳済み]

Here are the available Presentational Fields: [翻訳済み]
...
- [UI](../fields/ui) - blank field for custom UI components [翻訳済み]

### Virtual Fields [翻訳済み]

Virtual fields are used to display data that is not stored in the database. They are useful for displaying computed values that populate within the API response through hooks, etc. [翻訳済み]

Here are the available Virtual Fields: [翻訳済み]

- [Join](../fields/join) - achieves two-way data binding between fields [翻訳済み]
...
The following field names are forbidden and cannot be used: [翻訳済み]
...
### Field-level Hooks [翻訳済み]

In addition to being able to define [Hooks](../hooks/overview) on a document-level, you can define extremely granular logic field-by-field. [翻訳済み]
...
For full details on Field-level Hooks, see the [Field Hooks](../hooks/fields) documentation. [翻訳済み]

### Field-level Access Control [翻訳済み]

In addition to being able to define [Access Control](../access-control/overview) on a document-level, you can define extremely granular permissions field-by-field. [翻訳済み]
...
For full details on Field-level Access Control, see the [Field Access Control](../access-control/fields) documentation. [翻訳済み]
...
Functions can be written to make use of the following argument properties: [翻訳済み]
...
<Banner type="success">
  **Tip:** You can use async __PROTECTED_BLOCK_57__ functions to fill fields with data
  from API requests or Local API using __PROTECTED_BLOCK_58__.
</Banner>
...
#### Reusing Default Field Validations [翻訳済み]
...
To reuse default field validations, call them from within your custom validation function: [翻訳済み]
...
Here is a list of all default field validation functions: [翻訳済み]
...
#### Async Field Validations [翻訳済み]

Custom validation functions can also be asynchronous depending on your needs. This makes it possible to make requests to external services or perform other miscellaneous asynchronous logic. [翻訳済み]
...
## Custom ID Fields [翻訳済み]

All [Collections](../configuration/collections) automatically generate their own ID field. If needed, you can override this behavior by providing an explicit ID field to your config. This field should either be required or have a hook to generate the ID dynamically. [翻訳済み]
...
<Banner type="warning">
  **Reminder:** The Custom ID Fields can only be of type [__PROTECTED_BLOCK_93__](./number) or
  [__PROTECTED_BLOCK_94__](./text). Custom ID fields with type __PROTECTED_BLOCK_95__ must not contain __PROTECTED_BLOCK_96__ or
  __PROTECTED_BLOCK_97__ characters.
</Banner>
...
### Field Descriptions [翻訳済み]

Field Descriptions are used to provide additional information to the editor about a field, such as special instructions. Their placement varies from field to field, but typically are displayed with subtle style differences beneath the field inputs. [翻訳済み]

A description can be configured in three ways: [翻訳済み]
...
<Banner type="warning">
  **Reminder:** To replace the Field Description with a [Custom
  ...
  __PROTECTED_BLOCK_125__ property. [More details](#description).
</Banner>

#### Description Functions [翻訳済み]

Custom Descriptions can also be defined as a function. Description Functions are executed on the server and can be used to format simple descriptions based on the user's current [Locale](../configuration/localization). [翻訳済み]
...
All Description Functions receive the following arguments: [翻訳済み]
...
Within the [Admin Panel](../admin/overview), fields are represented in three distinct places: [翻訳済み]
...
- [Diff](#diff) - The Diff component rendered in the Version Diff View [翻訳済み]
...
The Field Component is the actual form field rendered in the Edit View. This is the input that user's will interact with when editing a document. [翻訳済み]
...
_For details on how to build Custom Components, see [Building Custom Components](../custom-components/overview#building-custom-components)._ [翻訳済み]

<Banner type="warning">
  Instead of replacing the entire Field Component, you can alternately replace
  or slot-in only specific parts by using the [__PROTECTED_BLOCK_152__](#label),
  [__PROTECTED_BLOCK_153__](#error), [__PROTECTED_BLOCK_154__](#afterinput-and-beforinput), and
  [__PROTECTED_BLOCK_155__](#afterinput-and-beforinput) properties.
</Banner>

##### Default Props [翻訳済み]

All Field Components receive the following props by default: [翻訳済み]
...
In addition to the above props, all Server Components will also receive the following props: [翻訳済み]
...
##### Sending and receiving values from the form [翻訳済み]
...
The Cell Component is rendered in the table of the List View. It represents the value of the field when displayed in a table cell. [翻訳済み]
...
All Cell Components receive the same [Default Field Component Props](#field), plus the following: [翻訳済み]
...
For details on how to build Custom Components themselves, see [Building Custom Components](../custom-components/overview#building-custom-components). [翻訳済み]

#### Filter [翻訳済み]

The Filter Component is the actual input element rendered within the "Filter By" dropdown of the List View used to represent this field when building filters. [翻訳済み]
...
All Custom Filter Components receive the same [Default Field Component Props](#field). [翻訳済み]

For details on how to build Custom Components themselves, see [Building Custom Components](../custom-components/overview#building-custom-components). [翻訳済み]

#### Label [翻訳済み]

The Label Component is rendered anywhere a field needs to be represented by a label. This is typically used in the Edit View, but can also be used in the List View and elsewhere. [翻訳済み]
...
All Custom Label Components receive the same [Default Field Component Props](#field). [翻訳済み]
...
Alternatively to the [Description Property](#field-descriptions), you can also use a [Custom Component](../custom-components/overview) as the Field Description. This can be useful when you need to provide more complex feedback to the user, such as rendering dynamic field values or other interactive elements. [翻訳済み]
...
All Custom Description Components receive the same [Default Field Component Props](#field). [翻訳済み]

For details on how to build a Custom Components themselves, see [Building Custom Components](../custom-components/overview#building-custom-components). [翻訳済み]
...
The Error Component is rendered when a field fails validation. It is typically displayed beneath the field input in a visually-compelling style. [翻訳済み]
...
All Error Components receive the [Default Field Component Props](#field). [翻訳済み]
...
The Diff Component is rendered in the Version Diff view. It will only be visible in entities with versioning enabled, [翻訳済み]
...
#### afterInput and beforeInput [翻訳済み]

With these properties you can add multiple components _before_ and _after_ the input element, as their name suggests. This is useful when you need to render additional elements alongside the field without replacing the entire field component. [翻訳済み]
...
For details on how to build Custom Components, see [Building Custom Components](../custom-components/overview#building-custom-components). [翻訳済み]
