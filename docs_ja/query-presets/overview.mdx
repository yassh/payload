...
label: 概要
order: 10
desc: Query Presets allow you to save and share filters, columns, and sort orders for your collections. [翻訳済み]
---
Query Presets allow you to save and share filters, columns, and sort orders for your [Collections](../configuration/collections). This is useful for reusing common or complex filtering patterns and/or sharing them across your team. [翻訳済み]
...
Within the [Admin Panel](../admin/overview), Query Presets are applied to the List View. When enabled, new controls are displayed for users to manage presets. Once saved, these presets can be loaded up at any time and optionally shared with others. [翻訳済み]
...
While not required, you may want to customize the behavior of Query Presets to suit your needs, such as add custom labels or access control rules. [翻訳済み]
...
The following options are available for Query Presets: [翻訳済み]
...
Access Control for Query Presets can be customized in two ways: [翻訳済み]
...
2. [Document Access Control](#document-access-control): Applies to each individual preset. These rules are controllable by the user and are dynamically defined on each record in the database. [翻訳済み]

### Collection Access Control [翻訳済み]

Collection-level access control applies to _all_ presets within the Query Presets collection. Users cannot control these rules, they are written statically in your config. [翻訳済み]
...
### Document Access Control [翻訳済み]

You can also define access control rules that apply to each specific preset. Users have the ability to define and modify these rules on the fly as they manage presets. These are saved dynamically in the database on each record. [翻訳済み]

When a user manages a preset, document-level access control options will be available to them in the Admin Panel for each operation. [翻訳済み]
...
- **Specific Users**: Only select users can read, update, and delete the preset. [翻訳済み]

#### Custom Access Control [翻訳済み]

You can augment the default access control rules with your own custom rules. This can be useful for creating more complex access control patterns that the defaults don't provide, such as for RBAC. [翻訳済み]

Adding custom access control rules requires: [翻訳済み]

1. A label to display in the dropdown
2. A set of fields to conditionally render when that option is selected
3. A function that returns the access control rules for that option [翻訳済み]
...
<Banner type="warning">
  **Note:** Payload places your custom fields into the __PROTECTED_BLOCK_16__ field
  group, so your rules will need to reflect this.
</Banner>

The following options are available for each constraint: [翻訳済み]
...
### Constraint Access Control [翻訳済み]

Used to dynamically filter which constraints are available based on the current user, document data, or other criteria. [翻訳済み]

Some examples of this might include: [翻訳済み]

- Ensuring that only "admins" are allowed to make a preset available to "everyone"
- Preventing the "onlyMe" option from being selected based on a hypothetical "disablePrivatePresets" checkbox [翻訳済み]

When a user lacks the permission to set a constraint, the option will either be hidden from them, or disabled if it is already saved to that preset. [翻訳済み]
